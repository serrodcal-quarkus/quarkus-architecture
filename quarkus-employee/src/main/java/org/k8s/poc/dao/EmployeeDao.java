...
public class EmployeeDao {
    ...
    private void initdb() {
        client.query("DROP TABLE IF EXISTS employees")
                .flatMap(r -> client.query("CREATE TABLE employees (id SERIAL PRIMARY KEY, name TEXT NOT NULL, dept_id BIGINT)"))
    ...
    public Uni<Employee> findById(Long id) {
        return client.preparedQuery("SELECT id, name, dept_id FROM employees WHERE id = $1", Tuple.of(id))
                .onItem().apply(RowSet::iterator)
                .onItem().apply(iterator -> iterator.hasNext() ? from(iterator.next()) : null);
    }

    public Multi<Employee> findByDept(Long depId) {
        return client.preparedQuery("SELECT id, name, dept_id FROM employees WHERE dept_id = $1", Tuple.of(depId))
                .onItem().produceMulti(set -> Multi.createFrom().items(() -> StreamSupport.stream(set.spliterator(), false)))
                .onItem().apply(EmployeeDao::from);
    }

    public Uni<Long> save(String name, Long deptId) {
        return client.preparedQuery("INSERT INTO employees (name, dept_id) VALUES ($1, $2) RETURNING (id)", Tuple.of(name, deptId))
                .onItem().apply(pgRowSet -> pgRowSet.iterator().next().getLong("id"));
    }

    public Uni<Boolean> update(Long id, String name, Long deptId) {
        return client.preparedQuery("UPDATE employees SET name = $1, dept_id = $3 WHERE id = $2", Tuple.of(name, id, deptId))
